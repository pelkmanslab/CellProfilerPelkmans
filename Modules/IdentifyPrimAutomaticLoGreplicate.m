...
%textVAR17 = Second LoG filter to resolve undersegmentation: Size of neighborhood(height,width),Sigma,Threshold. (First try: Sigma/2,-Threshold)
%defaultVAR17 = /
LaplaceTwoValues = char(handles.Settings.VariableValues{CurrentModuleNum,17});

%textVAR18 = LoG highlight correction: Upper fraction [0 to 1] of foreground pixels to be darkened. 0=Off.
%defaultVAR18 = 0
ItHighlightComp = str2num(char(handles.Settings.VariableValues{CurrentModuleNum,18}));

%textVAR19 = What do you want to call the outlines of the identified objects (optional)?
%defaultVAR19 = Do not save
%infotypeVAR19 = outlinegroup indep
SaveOutlines = char(handles.Settings.VariableValues{CurrentModuleNum,19});

%textVAR20 = Do you want to fill holes in identified objects?
%choiceVAR20 = Yes
%choiceVAR20 = No
FillHolesOption = char(handles.Settings.VariableValues{CurrentModuleNum,20});
...
%%% Checks that the Laplace parameters have valid values
if ~strcmp(LaplaceTwoValues,'/')
    index = strfind(LaplaceTwoValues,',');
    if isempty(index) || (length(index) ~= 3)
        error(['Image processing was canceled in the ', ModuleName, ' module because the Second Laplace Values are invalid.']);
    end
    
    NeighborhoodSizeTwo(1) = str2num(LaplaceTwoValues(1:index(1)-1));
    NeighborhoodSizeTwo(2) = str2num(LaplaceTwoValues(index(1)+1:index(2)-1));
    SigmaTwo = str2num(LaplaceTwoValues(index(2)+1:index(3)-1));
    LaplaceThresholdTwo = str2num(LaplaceTwoValues(index(3)+1:end));
end
...
for LocalMaximaTypeNumber = 1:length(LocalMaximaTypeList)
    for WatershedTransformImageTypeNumber = 1:length(WatershedTransformImageTypeList)
        ...
        if strcmp(LaplaceValues,'/')
            ...
            %%% Filters the image.
            imLoGout=imfilter(double(OrigImage),rgLoG,'replicate');
            %%% Removes noise using the weiner filter.
            imLoGoutW=wiener2(imLoGout,WienerSize);

            rgNegCurve = imLoGoutW < LaplaceThreshold;
            
            %%%Modification: iterative highlight compensation
            
            if ((ItHighlightComp>0)&&(ItHighlightComp<=1))
                FgPixels=sort(OrigImage(rgNegCurve));
                HighlightLevel=ItHighlightComp;%take upper fraction of pixels as highlights
                HighlightTheshold=FgPixels(round(length(FgPixels)*(1-HighlightLevel)));
                HCompImage=OrigImage;
                HCompImage(HCompImage>HighlightTheshold)=HighlightTheshold;
                
                %%% Filters the compensated image again
                imLoGout=imfilter(double(HCompImage),rgLoG,'replicate');
                %%% Removes noise using the weiner filter.
                imLoGoutW=wiener2(imLoGout,WienerSize);
            ...
            %%%Modification: Optional second LoG to prevent undersegmentation
            if ~strcmp(LaplaceTwoValues,'/')
                secLoG=fspecial('log',NeighborhoodSizeTwo,SigmaTwo);
                secimLoGout=imfilter(double(OrigImage),secLoG,'replicate');    
                %Masking with the previous result allows positive and negative thresholds as well as smaller sigmas to be used
                if strcmp(FillHolesOption,'Yes')
                   rgNegCurve=(secimLoGout<LaplaceThresholdTwo).*rgNegCurve; 
                   padrgNegCurve=padarray(rgNegCurve,[1 1],1);%make a virtual border so holes at the border are closed as well
                   padrgNegCurve([1 2 end-1 end],[1 2 end-1 end])=0;%open up all 4 corners to avoid filling up the whole image
                   padrgNegCurve=imfill(padrgNegCurve,'holes');
                   rgNegCurve=padrgNegCurve(2:end-1,2:end-1);%de pad
                else
                   rgNegCurve=(secimLoGout<LaplaceThresholdTwo).*rgNegCurve; 
                end
            end
            ...
            %dilates while preserving object labels
            %objects may now directly touch each other -> need smart
            %relabeling to avoid fusing objects as with bwlabel(im2bw())
            smartDilated=CPrgsmartdilate(rgLabelled,2);
            if strcmp(FillHolesOption,'Yes')
                rgDilated=imfill(smartDilated,'holes');
            else
                rgDilated=smartDilated;
            end
            
            PrelimLabelMatrixImage1=double(rgDilated);
            ...
            PrelimLabelMatrixImage2(AreasImage < (pi*(MinDiameter/2)^2)) = 0;%Modification: MinMaxAreas are true areas calculated from diamteres now
            ...
            if MaxDiameter ~= 99999
                PrelimLabelMatrixImage3(AreasImage > (pi*(MaxDiameter/2)^2)) = 0;%Modification: MinMaxAreas are true areas calculated from diamteres now
            ...
            if strncmpi(ExcludeBorderObjects,'Y',1) == 1
                Objects = CPclearborder(PrelimLabelMatrixImage3);
            else
                Objects = PrelimLabelMatrixImage3;
            end
            
            %Re label to get consecutive labels since objects may have been
            %removed based on area or edge location
            
            %[AS]smartrelabel to get continuous labels without using bwlabel(im2bw(...)) to avoid fusing adjacent objects
            OldLabels=unique(Objects);
            NewLabels(OldLabels+1)=0:length(OldLabels)-1;
            Objects=NewLabels(Objects+1);
            clear OldLabels NewLabels;
            NumOfObjects=length(unique(Objects))-1;
