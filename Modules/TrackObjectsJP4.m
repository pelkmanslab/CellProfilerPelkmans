...
if SetBeingAnalyzed == StartingImageSet
    ...
    switch lower(TrackingMethod)
        case 'distance'
            ...
            [CurrentLabels, CurrHeaders, ParentMat] = EvaluateObjectOverlap(CurrentObjNhood,PreviousObjNhood,PreviousLabels,PrevHeaders,MeasuredValues,OverlapFactorC,OverlapFactorP);
            
        case 'overlap'  % Compute object area overlap
            MeasuredValues = ones(size(CurrentSegmentedImage));
            [CurrentLabels, CurrHeaders, ParentMat] = EvaluateObjectOverlap(CurrentSegmentedImage,PreviousSegmentedImage,PreviousLabels,PrevHeaders,MeasuredValues,OverlapFactorC,OverlapFactorP);
...
if CollectStatistics
    ...
    handles = CPaddmeasurements(handles, ObjectName, TrackingMeasurementPrefix , CPjoinstrings('ObjectID',num2str(PixelRadius)), ...
                    CurrentLabels(:));
    if SetBeingAnalyzed == StartingImageSet
        handles = CPaddmeasurements(handles, ObjectName, TrackingMeasurementPrefix , CPjoinstrings('ParentID',num2str(PixelRadius)), ...
            CurrentLabels(:));
    else
        handles = CPaddmeasurements(handles, ObjectName, TrackingMeasurementPrefix , CPjoinstrings('ParentID',num2str(PixelRadius)), ...
            ParentMat(:));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [CurrentLabels, CurrentHeaders,mainParents2] = EvaluateObjectOverlap(CurrentLabelMatrix, PreviousLabelMatrix, PreviousLabels, PreviousHeaders, ChosenMetric,OverlapFactorC,OverlapFactorP)
...
if any(CurrentPreviousLabelHistogram(:)),
    ...
    mainParents2=[];
    for j=1:length(mainChildren)
        if mainChildren(j)==0
        else
           mainParents2(mainChildren(j),1)=mainParents(j);
        end
    end
