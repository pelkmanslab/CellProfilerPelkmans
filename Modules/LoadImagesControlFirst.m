function handles = LoadImagesControlFirst(handles)
...
%textVAR15 = Type the regexp that all control images have in common (such as H\d\d for row H, or [A-Z]01 for column 1):
%defaultVAR15 = H\d\d
CtrlTextToFind = char(handles.Settings.VariableValues{CurrentModuleNum,15});
    ...
    if strcmp(LoadChoice,'Order')
        ...
        if strcmp(ImageOrMovie,'Image')
            ...
            for n = 1:length(ImageName)
                ...
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                % BS: Seems like a good place to sort it such that the
                % control wells are analysed first! (I know, I still don't
                % make propes use of MatLab array sorting stuff... :-) 
%                 try
                    [cellStart cellEnd cellExtents cellMatch cellTokens cellNames] = regexp(FileList, CtrlTextToFind);
                    if not(isempty(cellMatch))
                        cellControlList = {};
                        cellNonControlList = {};                        
                        for iList = 1:length(FileList)
                            if isempty(cellMatch{iList})
                                cellNonControlList{1,end+1} = FileList{iList};
                            else
                                cellControlList{1,end+1} = FileList{iList};
                            end
                        end
                        if length(cellControlList) == 0
                            error(['[BS] WARNING: CONTROL IMAGES NOT FOUND!']);
                        end                        
                        ReSortedFileList = cat(2,cellControlList,cellNonControlList);
                        FileList = ReSortedFileList;
                    end
...
for n = 1:length(ImageName)
    if strcmp(ImageOrMovie,'Image')
        ...
        try
            ...
            %%% [BS] Disabled because its anoying!
