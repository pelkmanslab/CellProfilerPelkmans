function handles = IdentifyPrimLoGAndCalculateEnrichment_developing(handles)
...
% % % 
% % % %textVAR05 = Distance between reference ring and vesicle, in pixel units
% % % %defaultVAR05 = 10
% % % ReferenceDistance = char(handles.Settings.VariableValues{CurrentModuleNum,3});
% % % 
% % % %textVAR06 = Reference ring width, in pixel units
% % % %defaultVAR06 = 5
% % % ReferenceWidth = char(handles.Settings.VariableValues{CurrentModuleNum,3});
% % % 
% % % %textVAR07 = Score threshold for match.  Enter a number, leave as 'Automatic', or adjust the Automatic threshold with a multiplicative correction factor (separating comma is necessary), e.g. 'Automatic,1.2'.
...
% % % 
% % % %textVAR08 = On what images do you want to calculate enrichment?
% % % %infotypeVAR08 = imagegroup
% % % EnrichmentImageName = char(handles.Settings.VariableValues{CurrentModuleNum,1});
...
% % % ReferenceDistance = str2double(ReferenceDistance);
% % % ReferenceWidth = str2double(ReferenceWidth);
...
ImageName = 'OrigRed';
ObjectName = 'Vesicles';
ParentObjectName = 'Cells';

OrigImage = imread('C:\Users\Pelkmans\Desktop\Temp_CP_dev_images\TIFF\density dependent ChtxB uptake_D08_s19_w2E2C310A5-882F-43FB-8490-4ACAA0770AAE.png');
EnrichmentImage = imread('C:\Users\Pelkmans\Desktop\Temp_CP_dev_images\TIFF\density dependent ChtxB uptake_D08_s19_w3475B3021-B91F-47E4-A045-CAFFA98629AD.png');

ThresholdStr = 'Automatic,1.6';

% ParentLabelMatrixImage = CPretrieveimage(handles,['Segmented', PrimaryObjectName],ModuleName,'DontCheckColor','DontCheckScale',size(OrigImage));
ParentLabelMatrixImage = imread('C:\Users\Pelkmans\Desktop\Temp_CP_dev_images\SEGMENTATION\density dependent ChtxB uptake_D08_s19_w1006D589E-B072-4D91-B020-71893132C045_SegmentedCells.png');
...
sigma = (Radius-1)/3;
op = fspecial('log',Radius,sigma); 
op = op - sum(op(:))/numel(op); % make the op to sum to zero

%% Pad image to fix border artifact
padsize = ceil(Radius./2);
pad_im = padarray(im,[padsize padsize],'replicate');

%% Use 'single' here, since imfilter is optimized on Intel chips for
%% certain array classes (see Matlab documentation)
ac = double(imfilter(single(pad_im),op));

%% Crop pad
ac = ac(padsize+1:end-padsize,padsize+1:end-padsize);
clear sigma op padsize pad_im
%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
elseif strcmpi('Automatic',ThresholdStr(1:9))
    ...
    % local background
    fun = @(block_struct) RobustBackground_BS(block_struct.data);
    Threshold = blockproc(ac_scaled,(size(ac_scaled) ./ [20,24]),fun);
    Threshold2 = imresize(Threshold,size(ac_scaled));
    % first, rescale to 0 - 1
    Threshold2 = Threshold2 - min(Threshold2(:));
    Threshold2 = Threshold2 / max(Threshold2(:));
    
    Threshold = RobustBackground_BS(ac_scaled);
    
    
    Threshold = Threshold * Threshold_correction;
    clear Threshold2
...
% get reference dots, i.e. pseudo-random dots, evenly spaced at Radius interval
% 1) make neatly ordered grid
reference_dots = false(size(im));
reference_dots(1:Radius:end,1:Radius:end) = true;
...
matEnrichmentScorePerParent = NaN(intNumOfParentObjects,4);
