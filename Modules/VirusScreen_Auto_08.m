function handles = VirusScreen_Auto_08(handles)
...
%%%VariableRevisionNumber = 1
    ...
    strIntensityMeasurementList = '2,7';
...
%     try
        ...
        if intTotalCells > 1        
            ...
            for iIntensity = 1:length(matIntensityMeasurements)
                ...
                matCurrentIntensityMeasurement = log10(IntensityMeasurements(:,matIntensityMeasurements(1,iIntensity)));
                
                [cellGaussians{iIntensity,1} cellGaussians{iIntensity,2} cellGaussians{iIntensity,3}] = fit_mix_gaussian(matCurrentIntensityMeasurement, intNumberOfGaussians);
                ...
                if isnan(matIntensityThresholds(1,iIntensity))
                    if length(cellIncludedThresholds{iIntensity}) > 0
                        matPreliminaryHits{iIntensity} = find(matCurrentIntensityMeasurement > mean(cellIncludedThresholds{iIntensity}));
                    ...
                    if length(cellIncludedThresholds{iIntensity}) > 0
                        ...
                        elseif (matIntensityThresholds(1,iIntensity) < min(cellIncludedThresholds{iIntensity}))
                            ...
                            matPreliminaryHits{iIntensity} = find(matCurrentIntensityMeasurement > matIntensityThresholds(1,iIntensity));
        ...
        if intWindowOpenForNumberOfCycles == 60
            ...
            if intTotalCells > 1
                ...
                for iPlots = 1:length(matIntensityMeasurements)
                    ...
                    if intNumberOfGaussians == 1
                        plot_mix_gaussian2(cellGaussians{iPlots,1},cellGaussians{iPlots,2},[1],log10(IntensityMeasurements(:,matIntensityMeasurements(1,iPlots))),4,4,iPlots,strPlotName,[matIntensityThresholds(1,iPlots), mean(cellIncludedThresholds{iPlots}), min(cellIncludedThresholds{iPlots})]);
                    else
                        plot_mix_gaussian2(cellGaussians{iPlots,1},cellGaussians{iPlots,2},[0.5, 0.5],log10(IntensityMeasurements(:,matIntensityMeasurements(1,iPlots))),4,4,iPlots,strPlotName,[matIntensityThresholds(1,iPlots), mean(cellIncludedThresholds{iPlots}), min(cellIncludedThresholds{iPlots})]);
                    end
                    %plot-subscript
