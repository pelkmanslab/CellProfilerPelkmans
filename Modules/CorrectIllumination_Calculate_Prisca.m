function handles = CorrectIllumination_Calculate_Prisca(handles)
...
%textVAR14 = For RESCALED IMAGES: Enter the rescaling value
%defaultVAR14 = 0.002
RescalingValue = str2double(char(handles.Settings.VariableValues{CurrentModuleNum,14}));
    ...
    try
        if strcmp(SourceIsLoadedOrPipeline, 'Load Images module') == 1 && handles.Current.SetBeingAnalyzed == 1
            ...
            if strcmp(IntensityChoice,'Regular')
                ...
                [handles, RawImage, ReadyFlag] = CPaverageimages(handles, 'DoNow', ImageName, 'ignore', RescalingValue);
