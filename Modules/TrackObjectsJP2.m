...
%textVAR12 = What is the relative overlap of Parent on Child you would like qualify as being related?
%defaultVAR12 = Do not use
%infotypeVAR12 = imagegroup indep
OverlapFactorC = char(handles.Settings.VariableValues{CurrentModuleNum,12});

%textVAR13 = What is the relative overlap of Child on Parent you would like qualify as being related?
%defaultVAR13 = Do not use
%infotypeVAR13 = imagegroup indep
OverlapFactorP = char(handles.Settings.VariableValues{CurrentModuleNum,13});
...
if SetBeingAnalyzed == StartingImageSet
    ...
    switch lower(TrackingMethod)
        case 'distance'
            ...
            [CurrentLabels, CurrHeaders] = EvaluateObjectOverlap(CurrentObjNhood,PreviousObjNhood,PreviousLabels,PrevHeaders,MeasuredValues,OverlapFactorC,OverlapFactorP);
            
        case 'overlap'  % Compute object area overlap
            MeasuredValues = ones(size(CurrentSegmentedImage));
            [CurrentLabels, CurrHeaders] = EvaluateObjectOverlap(CurrentSegmentedImage,PreviousSegmentedImage,PreviousLabels,PrevHeaders,MeasuredValues,OverlapFactorC,OverlapFactorP);
...
if isOpenAssignedModuleFigure || (~isOpenAssignedModuleFigure && userWantsDataImage )
    ...
    if isOpenAssignedModuleFigure
        ...
        hAx = CPimagesc(CurrentColoredLabelImage,handles)
        title(['Tracked ',ObjectName]);
...
%             'fontweight','bold');%,'Parent',hAx); Note need to find the solution for this
    end
end
...
if CollectStatistics
    ...
    handles = CPaddmeasurements(handles, ObjectName, TrackingMeasurementPrefix , CPjoinstrings('ObjectID',num2str(PixelRadius)), ...
                    CurrentLabels(:));
    handles = CPaddmeasurements(handles, ObjectName, TrackingMeasurementPrefix, CPjoinstrings('ProgenyID',num2str(PixelRadius)), ...
                    CurrHeaders(:));
    handles = CPaddmeasurements(handles, ObjectName, TrackingMeasurementPrefix, CPjoinstrings('TrajectoryX',num2str(PixelRadius)), ...
                    CentroidTrajectory(:,1));
    handles = CPaddmeasurements(handles, ObjectName, TrackingMeasurementPrefix, CPjoinstrings('TrajectoryY',num2str(PixelRadius)), ...
                    CentroidTrajectory(:,2));
    handles = CPaddmeasurements(handles, ObjectName, TrackingMeasurementPrefix, CPjoinstrings('DistanceTraveled',num2str(PixelRadius)), ...
                    DistanceTraveled(:));
    ...
    %if SetBeingAnalyzed ~= NumberOfImageSets,
		...
		handles = CPaddmeasurements(handles, 'Image', TrackingMeasurementPrefix, CPjoinstrings('LostObjectCount',ObjectName,num2str(PixelRadius)), ...
		...
		handles = CPaddmeasurements(handles, 'Image', TrackingMeasurementPrefix, CPjoinstrings('NewObjectCount',ObjectName,num2str(PixelRadius)), ...
                    length(NewObjectsLabel));
    %else %... or we reach the end of the analysis
         
    
    if SetBeingAnalyzed ~= StartingImageSet
        IntegratedDistanceMeasurementName = CPjoinstrings('IntegratedDistance',num2str(PixelRadius));
        handles = CPaddmeasurementsTracking(handles, ObjectName, TrackingMeasurementPrefix, IntegratedDistanceMeasurementName, ...
                        IntegratedDistance(:));
        LinearityMeasurementName = CPjoinstrings('Linearity',num2str(PixelRadius));
        handles = CPaddmeasurementsTracking(handles, ObjectName, TrackingMeasurementPrefix, LinearityMeasurementName, ...
                        Linearity(:));
        LifetimeMeasurementName = CPjoinstrings('Lifetime',num2str(PixelRadius));
        handles = CPaddmeasurementsTracking(handles, ObjectName, TrackingMeasurementPrefix, LifetimeMeasurementName, ...
                        Lifetime(:));
    end
    ...
    if SetBeingAnalyzed == NumberOfImageSets,
        ...
        IntegratedDistanceMeasurementName = CPjoinstrings('IntegratedDistance',num2str(PixelRadius));
        handles = CPaddmeasurements(handles, ObjectName, TrackingMeasurementPrefix, IntegratedDistanceMeasurementName, ...
                        IntegratedDistance(:));
        LinearityMeasurementName = CPjoinstrings('Linearity',num2str(PixelRadius));
        handles = CPaddmeasurements(handles, ObjectName, TrackingMeasurementPrefix, LinearityMeasurementName, ...
                        Linearity(:));
        LifetimeMeasurementName = CPjoinstrings('Lifetime',num2str(PixelRadius));
        handles = CPaddmeasurements(handles, ObjectName, TrackingMeasurementPrefix, LifetimeMeasurementName, ...
                        Lifetime(:));         
    ...
    % measurements
    %%% Note by Nico: This step can be expcludeed in this version
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [CurrentLabels, CurrentHeaders] = EvaluateObjectOverlap(CurrentLabelMatrix, PreviousLabelMatrix, PreviousLabels, PreviousHeaders, ChosenMetric,OverlapFactorC,OverlapFactorP)
...
if any(CurrentPreviousLabelHistogram(:)),
    ...
    %Generate a list of the most likely children which will later be
    %renamed equal to their parent
    ...
    %vectorise parents and children
    ParentsMatrix=ParentsMatrix(CurrentObjList>0);
    CurrentObjList=CurrentObjList(CurrentObjList>0);
...
CurrentObjList=vertcat(CurrentObjList,idx);

CurrentLabels=CurrentObjList;

CurrentHeaders=[ParentsMatrix;145];
