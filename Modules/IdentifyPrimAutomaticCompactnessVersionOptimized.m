...
%textVAR20 = Use Cell divider to divide undersegmented cells?
%choiceVAR20 = Yes
%choiceVAR20 = No
cellDiv = char(handles.Settings.VariableValues{CurrentModuleNum,20});
%inputtypeVAR20 = popupmenu

%textVAR21 = Cell divider maximal threshold factor: defines the upper bound used by the CellDivider to threshold the image in order to find undersegmented cells. If this is to low, undersegmented cells could be missed, but speed is a little bit increased.
%defaultVAR21 = 2.5
ThreshFacMax = str2double(char(handles.Settings.VariableValues{CurrentModuleNum,21}));


%textVAR22 = Cell divider minimum size: is the the minimal size in pixel^2 an object has to have in order to be not discarded in thresholded image. Oversegmentation can happen if this value is to low.
%defaultVAR22 = 300
Minsize = str2double(char(handles.Settings.VariableValues{CurrentModuleNum,22}));

%textVAR23 = Cell divider Stepsize: is the increment between threshold levels used. Segmentation quality could increase if lowered, but speed decreases correspondingly.
%defaultVAR23 = 20
Stepsize = str2double(char(handles.Settings.VariableValues{CurrentModuleNum,23}));
...
for LocalMaximaTypeNumber = 1:length(LocalMaximaTypeList)
    for WatershedTransformImageTypeNumber = 1:length(WatershedTransformImageTypeList)
        ...
        if strcmp(LaplaceValues,'/')
            ...
            %%% Divide undersegmented nuclei (VZ added 111003)
            
            if strcmp(cellDiv,'Yes')
            
            Objects = CellDivider(Objects,OrigImage,Threshold,ThreshFacMax,Stepsize,Minsize);
        ...
        if strcmp(OriginalLocalMaximaType,'None') || (strcmp(OriginalLocalMaximaType,LocalMaximaType) && strcmp(OriginalWatershedTransformImageType,WatershedTransformImageType))
            ...
            if strcmp(LaplaceValues,'/')
                ...
                if any(findobj == ThisModuleFigureNumber) | ~strcmpi(SaveOutlines,'Do not save') %#ok Ignore MLint
                    ...
                    %%% image with watershed lines drawn to  up clusters of objects.
                    subplot(2,2,4);
...
% VitoZ
% This function cuts undersegmented cells according to the fact, whether two
% objects with a minimum size can be seen, when a higher threshold is
% applied.  
...
function Objects=CellDivider(Objects,origImage,threshold,threshFacMax,threshStepSize,minSize)

% VZ: Uses large parts of the EvaluateObjectOverlap which is part of the Basistraker
% function and largely based on CPrelateobjects

%Some Initialisations
lowerThreshold = threshold;

upperThreshold = threshFacMax * threshold;

threshStepSize = threshStepSize / 65535;

threshSteps = floor((upperThreshold - lowerThreshold)/(threshStepSize));

threshSteps = threshSteps +1;

ChosenMetric = ones(size(Objects));

thresh = lowerThreshold;

j=0;
anyObjLeft = 1;


% Filter away to small objects that can never be divided anyway.
objArea = regionprops(Objects,'Area');
smallObj = find([objArea.Area] < (minSize*2));

smallObjImage = ismember(Objects,smallObj);
Objects(smallObjImage) = 0;

Objects = bwlabel(Objects);


while j < threshSteps && anyObjLeft
    
    j = j+1;
    
    %Calculate the thresholded map:
    
    %threshold the Image
    threshMap = origImage > thresh;
    %segment it
    threshMap = bwlabel(threshMap);
    
    %increase the threshold
    thresh = thresh + threshStepSize;
    
    
    
    %Load the maps
    
    PreviousLabelMatrix = Objects;
    
    CurrentLabelMatrix = threshMap;
    
    %save Parents
    Parents=unique(PreviousLabelMatrix(PreviousLabelMatrix > 0));
    
    
    PreviousFamilyLabel = Parents;
    ...
    NumberOfPreviousObj = length(PreviousFamilyLabel);
    ...
    %if CurrentPreviousLabelHistogram(:) > minSize
    ...
    % Get the corresponding overlap list. Note as in this thresholded
    % images childs overlap allways 100% with the parents, this is
    % equivalent to the child area
    
    CurrentObjArea = OverlapCounts(end-MaxNumOverlap+1:end, :);
    ...
    CurrentObjArea(CurrentObjArea<minSize)=0;
    
    %Only allow object IDs which exist after CurrOverlapArea
    CurrentObjList(CurrentObjArea==0)=0;
    
    %Turn CurrObjList
    
    CurrentObjList = CurrentObjList';
    
    % Determine wether any child objs are left
    
    if ~any(CurrentObjList(:))
        anyObjLeft = 0;
    else
    
    %Identify cuts
    if size(CurrentObjList,2) <2
        divChildren = [];
    else
    divChildren=CurrentObjList(:,end-1);
    end
    
    
    
    %Determine the Id of the parent of the objects that should get cut
    PartoCut = (divChildren~=0);
    PartoCut = find(PartoCut);
    
    % Assign Family IDs
    
    [ObjRow, ~, ObjId]= find(CurrentObjList);
    CurrentFamilyLabel = zeros(max(ObjId),1);
    CurrentFamilyLabel(ObjId) = PreviousFamilyLabel(ObjRow);
    
    % Find which Objects to cut
    
    ObjtoCut =PreviousFamilyLabel(PartoCut);
    
    
    % Do the cutting
    
    for i=1:length(PartoCut)
        
        childObj = CurrentObjList(PartoCut(i),:);
        childObj(childObj == 0) = [];
        
        %create a new mask, containing only the two objects of the current
        %treshlevel map, which contain the child objects of the original
        %"mother" object
        
        childObjMapSel = ismember(CurrentLabelMatrix,childObj);
        childObjMap = CurrentLabelMatrix;
        childObjMap(not(childObjMapSel)) = 0;
        %The next few lines do the cutting
        %Expand the areas of the found nuclei pieces to the full image
        [bwtemp,bwtemplab] =bwdist(childObjMap);
        cutAreas = (bwtemp >= 0).*childObjMap(bwtemplab);
        
        %identify the edges between the expanded areas.
        cutBorder = edge_bs(cutAreas);
        cutPixels = Objects == ObjtoCut(i);
        cutPixels(not(cutBorder)) = 0;
        
        %cut the cells with help of the cutPixels
        Objects(cutPixels) = 0;
        
        %As a signal, that there was no division, this variable
        %is set to zero.
    end
...
%re-add the small objects previously removed
Objects(smallObjImage) = 1;
