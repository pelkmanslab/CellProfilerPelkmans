function handles = DiscardSinglePixelObjects_stephan2(handles)
...
%textVAR02 = What is the maximum allowed object area?
%defaultVAR02 = 150
MaxAreaSize = str2double(handles.Settings.VariableValues{CurrentModuleNum,2});

%textVAR03 = What is the maximum allowed eccentricity (e.g. 0 for circles, 1 for lines)?
%defaultVAR03 = 0.5
ValueEccentricity = str2double(handles.Settings.VariableValues{CurrentModuleNum,3});
...
%Adding of matrix for values of eccentricity
matObjectEccentricity = regionprops(LabelMatrixImage,'Eccentricity');
matObjectEccentricity = cat(1,matObjectEccentricity.Eccentricity)
...
% all variables MinAreaSizes are changed to MaxAreaSize and the < was
% changed to an > sign in order to exclude objects which are larger than
% the desired size. Since the data structure of the algorithm are the same,
% these are the only changes to be made to have a module to exclude larger
% objects.
% Since the Filter gives the object indices, it is easy also to add an
% additional constraint (other than size). Here Eccentricity was added to
% find all objects indices with an eccentricity value higher than the one
% chosen by the user.

Filter = find(matObjectSizes > MaxAreaSize  |  matObjectEccentricity > ValueEccentricity);
...
if any(findobj == ThisModuleFigureNumber)
    ...
    title(sprintf('Output label matrix (%s): max = %d (filtering objects smaller than %d pixels)',ObjectName,max(FinalLabelMatrixImage(:)),MaxAreaSize));
end
