...
if length(x)>1  % vector input
    ...
    if length(label)
        ...
        xunit=(x-xx(1))/xrange;
        if xunit<0.8
            text(x+0.01*xrange,y(1)+0.95*(y(2)-y(1)),label,'color',get(h,'color'),'rotation',-90,'fontsize',7)
        else
            text(x-.05*xrange,y(1)+0.95*(y(2)-y(1)),label,'color',get(h,'color'),'rotation',-90,'fontsize',7)
        end
    end     
