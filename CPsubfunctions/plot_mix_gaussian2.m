function plot_mix_gaussian2( u,sig,prob,X, plotx, ploty, plotnr, plottitle, threshhold )
%
...
if (size(u,1)==2) & (size(u,2)==size(prob,2))
    plot_mix_gaussian_2d( u,sig,prob,X, plotx, ploty, plotnr, plottitle,threshhold );
else
    plot_mix_gaussian_1d( u,sig,prob,X, plotx, ploty, plotnr, plottitle,nbins,threshhold );
end
drawnow;
...
function plot_mix_gaussian_1d( u,sig,prob,X, plotx, ploty, plotnr, plottitle, nbins,threshhold )
% plot normalized histogram and normalized distribution on top of it

% BS CP Hack
if not(isempty(plotx)) && not(isempty(ploty)) && not(isempty(plotnr))
    subplot(plotx,ploty,plotnr);
end
% constant
points = 1000;
...
% BS ADDED Stuff
matColors = ['r', 'b', 'y', 'm', 'c', 'r', 'b', 'y', 'm', 'c', 'r', 'b', 'y', 'm', 'c'];
title(plottitle, 'fontsize', 7);
%title(plottitle,'Color',matColors(1,plotnr));


matThreshholdColors = ['g', 'b', 'y', 'y', 'y'];
for i = 1:length(threshhold)
    if threshhold(i) < max(X)
%         if i > 2
            vline(threshhold(i),matThreshholdColors(i),'');
%         else
%             vline(threshhold(i),matThreshholdColors(i),num2str(threshhold(i)));        
%         end
    end
end
...
function plot_mix_gaussian_2d( u,covar,prob,X, plotx, ploty, plotnr, plottitle,threshhold )
% plot 2D samples and distribution data on top of it

% BS CP Hack
if not(isempty(plotx)) && not(isempty(plotx)) && not(isempty(plotx))
    subplot(plotx,ploty,plotnr);
end
...
if plotnr == 1
    title(plottitle, 'fontsize', 7,'Color','r');
else
    title(plottitle, 'fontsize', 7,'Color','b');
end
vline(threshhold,'g','Threshhold');