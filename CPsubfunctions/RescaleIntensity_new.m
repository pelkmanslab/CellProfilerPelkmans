...
%textVAR04 = (Method E only): Enter the intensity from the original image that should be set to the lowest value in the rescaled image, or type AA to calculate the lowest intensity automatically (BA peak estimator) from all of the images to be analyzed and AE to calculate the lowest intensity from each image independently.
%defaultVAR04 = AA
LowestPixelOrig = char(handles.Settings.VariableValues{CurrentModuleNum,4});

%textVAR05 = (Method E only): Enter the intensity from the original image that should be set to the highest value in the rescaled image, or type AA to calculate the highest intensity automatically (BS robust version) from all of the images to be analyzed and AE to calculate the highest intensity from each image independently.
%defaultVAR05 = AA
...
[handles,RescaledImage] = CPrescale2_Pauli(handles,OrigImage,RescaleOption,MethodSpecificArguments);
